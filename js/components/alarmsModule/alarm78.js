// Alarm78 (Wrong direction vehicle) details tabs
function openAlarm78DetStyle(evt, styleName) {
	var i, x, alarmDetailsTabs;
	x = $(".alarm78DetStyle");
	for (i = 0; i < x.length; i++) {
		x[i].style.display = "none";
	}
	alarmDetailsTabs = $(".alarm78DetailsTab");
	for (i = 0; i < x.length; i++) {
...
// Alarm1 events
$("#alarm78Event1").click(function () {
	$(this).parent('#alarm78DetStyle1 .alarmEventsRow').parent('.alarmModAlarmDetails.events').find('.alarmEventsDocument').removeClass("active");
	$(this).addClass('active');
	$('#alarm78DisplayControls').css('display', 'flex');
	$('#alarm78ModEventsDisplay').addClass('alarm78Event1');
	
	$('#alarm78DisplayControls').addClass('alarm78Event1');

	// $('#alarm78DisplayControls').children('.alarmsPlayPlayer.alarms').hide();
	$('#alarm78DisplayControls').children('.playAlarm78Event1').show();
});


$("#closeDocumentAlarm78").click(function () {
	$('#alarm78DisplayControls').css('display', 'none');
	$('#alarm78ModEventsDisplay').removeClass('alarm78Event1');
	$('#alarm78DetStyle1 .alarmEventsDocument').removeClass("active");
	$('#alarm78DisplayControls').removeClass('alarm78Event1');
});
...
// Alarm78 RTPs
$("#alarm78RTP1").click(function () {
	$(this).parent('.alarmEventsRow').parent('.alarmModAlarmDetails.events').find('.alarmRTPsDocument').removeClass("active");
	$(this).addClass('active');
	$('#alarm78RTPDisplayControls').css('display', 'flex');
	$('#alarm78RTPsDisplay').addClass('alarm78Event1');
	$('#alarm78RTPsDisplay').addClass('alarm78Event1');

	$('#alarm78RTPDisplayControls').children('.playAlarm78Event1').show();
});


$("#closeAlarm78RTP1").click(function () {
	$('#alarm78RTPDisplayControls').css('display', 'none');
	$('.alarmRTPsDocument').removeClass("active");
	$('#alarm78RTPsDisplay').removeClass('alarm78Event1');
	$('#alarm78RTPsDisplay').removeClass('alarm78Event1');
});